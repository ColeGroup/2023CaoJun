...
from model.cluster import KM, SC, SB, KMPP, AC
import sklearn.svm as svm
from sklearn import preprocessing
from utils import *

# def PC_GAIN(incomplete_data_x, gain_parameters, data_m):
def PC_GAIN(ori_data, incomplete_data_x, data_m, gain_parameters):
    ...
    with tf.Session() as sess:
        ...
        for it in tqdm(range(int(iterations * 0.7))):
            ...
            _, D_loss_curr, D_logit_curr, D_prob_curr = sess.run([D_solver, D_loss_temp, D_logit, D_prob],
                                                                 feed_dict={M: M_mb, X: X_mb, H: H_mb})
            _, G_loss_curr, MSE_loss_curr = sess.run([G_solver_pre, G_loss_temp, MSE_loss],
        ...
        for it in tqdm(range(iterations)):
            ...
            _, G_loss_curr, MSE_loss_curr, G_loss_with_C_curr = sess.run(
                [G_solver, G_loss_temp, MSE_loss, G_loss_with_C], feed_dict={X: X_mb, M: M_mb, H: H_mb, Y: sample_prob})
        ...
        # Rounding
        imputed_data = rounding(imputed_data, incomplete_data_x)

        rmse_gain = round(rmse_loss(ori_data.values, imputed_data, data_m), 5)
        print('pcgain: ', rmse_gain)
    return rmse_gain