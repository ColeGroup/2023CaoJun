...
import pylab as pl
from data.dataloader_2 import multi_view_data_handwritten
'''handwritten公共数据集的对比实验'''
...
for i in range(1, 10, 1):
    ...
    ori_data_x, miss_data_x, data_m \
        , miss_data_x1, miss_data_x2, miss_data_x3, miss_data_x4, miss_data_x5, miss_data_x6 = multi_view_data_handwritten(i)
    for epoch in range(0, 1):
        count = count + 1
        '''rf 1.0 '''
        # 多视角特征拼接后使用缺失森林填补
        imputed_RF_1 = miss_data_x.copy()
        rmse_rf_1 = RF_1_r(ori_data_x, imputed_RF_1, data_m,tree=10)
        rf1=rf1+rmse_rf_1
        # 对每个单独的视角使用缺失森林进行填补
        # '''rf 2.0'''
        # imputed_RF_2 = miss_data_x.copy()
        # rmse_rf_2 = RF_2_handwritten(ori_data_x, miss_data_x1, miss_data_x2, miss_data_x3,miss_data_x4, miss_data_x5, miss_data_x6, data_m)
        # rf2 = rf2 + rmse_rf_2
        # 基于特征相关性对每个视角构建随机森林预测缺失值
        # 然后基于特征重要性加权集成每个视角的缺失值得到最终得缺失值
        '''rf 3.0 '''
        imputed_RF_3 = miss_data_x.copy()
        rmse_rf_3 = RF_3_handwritten(ori_data_x, imputed_RF_3, data_m)
        rf3 = rf3 +rmse_rf_3
        '''KNN'''
